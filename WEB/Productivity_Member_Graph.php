...
$sql0 = "select * from $$UID ";
 $studentList0 = exResult($sql0);

 foreach ($studentList0 as $std0) {

    if($std0['THANG']==$month_n)
    // {echo "    Month of day:";
        // echo $month_n;
        // echo "    Month :";
        // echo $std0['THANG'];
        if(!in_array($std0['NGAY'],$Wday))
        {
            $Wday[$std0['NGAY']]=$std0['NGAY'];
            $Wtime[$std0['NGAY']]=$std0['Total_Time'];
            // echo "    WTIME:";
            // echo  $Wday[$std0['NGAY']];
            // echo "    WTIME:";
            // echo    $Wtime[$std0['NGAY']];
        }
        else
        {
            $Wtime[$std0['NGAY']]+=$std0['Total_Time'];
...
for ($i = 1; $i<=$day_n ;$i++){
   if($Wtime[$i]!=0){
               ...
               $dataPoints00=array("label"=> strval( $i) , "y"=>number_format ($Wtime[$i]/60 , 1 ,'.' , '' ) );
       print_r($dataPoints00);
                  $dataPoints[]=$dataPoints00;
          }
          else {
           
               $dataPoints00=array("label"=>  strval($i), "y"=>0 );
               $dataPoints[]=$dataPoints00;
          }
}
// print_r($dataPoints);
     ...
     <a target="_self"  href ="Homepage.php" id="bien" title="Ve Trang Dau" target="_blank"  >X </a>
		...
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h1 style=" color: #000000; "> Tổng quan </h1>
		
			</div>

			<script type="text/javascript">
	function onclick1(){
		window.location="Productivity_Member.php";	
	}
